...
{
	$CI= &get_instance();
	return  $CI->db->query("select GROUP_CONCAT(name) as name from hoa_community where id IN($id)")->row_array();
}
								...
								<div class='col-md-12'>
									<div class='form-group'>
										<div class='col-md-3 col-md-offset-5'>
											<input type='submit' class='btn btn-info' value='".$json->label."'>
										</div>
									</div>
								</div>						
							</li>";
...
function get_counts($table,$where='')
{
	...
	$CI->load->model("home_model");
	if($where!='')
			$where = array("usertype!="=>0);
	$result = $CI->home_model->get_counts($table,$where);
	return $result;
}
