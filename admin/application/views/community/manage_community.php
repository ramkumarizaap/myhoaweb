...
<ol class="breadcrumb bc-3">
	...
	<li class="active"><strong>Community Users</strong></li>
</ol>
					
<h2>Community List</h2>
<?php
if($this->session->flashdata('community_add')==TRUE)
{
	?>
<div class="alert alert-success alert-dismissable">
	<button class="close" aria-hidden="true" data-dismiss="alert">x</button>
	<?=$this->session->flashdata('community_add');?>
</div>
<?php 
}
else if($this->session->flashdata('community_fail')==TRUE)
{
	?>
	<div class="alert alert-danger alert-dismissable">
		<button class="close" aria-hidden="true" data-dismiss="alert">x</button>
		<?=$this->session->flashdata('community_fail');?>
	</div>
	<?php
}
?>
				...
				foreach ($community as $key => $value)
				{
					...
					if($value['status']==0)
					{
						$st = "<a style='color:lime'  title='Unpublish User'
										href='".base_url()."community/update_status/1/".$id."'>
										<i class='entypo-check'></i>
									</a>";
					}
					else
					{
						$st = "<a style='color:red' href='".base_url()."community/update_status/0/".$id."'>
